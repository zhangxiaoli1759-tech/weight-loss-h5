...
移动端优先的减肥打卡 Web 应用：每日记录体重 / 体脂率 / 入睡时间 / 泡脚 / 偷吃 / 运动，自动计算预估体重曲线、与昨日对比、距目标剩余天数与每日需减进度，并用图表展示体重与体脂变化趋势。
...
## 功能
1. **每日打卡**：点「今日打卡」按钮弹出表单。体重（2 位小数）、体脂率（2 位小数，选填）、入睡时间（默认 23:00 可改）、泡脚（默认是）、偷吃（默认否，与泡脚同一行）、运动（默认无，选「有」弹出行时长）。
